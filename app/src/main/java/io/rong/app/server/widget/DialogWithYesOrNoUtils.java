package io.rong.app.server.widget;
        ...
        dialog.setPositiveButton("确认修改", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (TextUtils.isEmpty(old)) {
                    NToast.shortToast(context, "初始密码不能为空");
                    return;
                }
                if (TextUtils.isEmpty(new1)) {
                    NToast.shortToast(context, "新密码不能为空");
                    return;
                }
                if (TextUtils.isEmpty(new2)) {
                    NToast.shortToast(context, "确认密码不能为空");
                    return;
                }
                if (!cachePassword.equals(old)) {
                    NToast.shortToast(context, "原始密码不正确");
                    return;
                }
                if (!new1.equals(new2)) {
                    NToast.shortToast(context, "两次确认密码不一致");
                    return;
                }
