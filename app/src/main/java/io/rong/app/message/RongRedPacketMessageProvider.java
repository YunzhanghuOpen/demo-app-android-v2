...
public class RongRedPacketMessageProvider extends IContainerItemProvider.MessageProvider<RongRedPacketMessage> {
	...
	public View newView(Context context, ViewGroup group) {
		...
		holder.message = (TextView) view.findViewById(R.id.tv_money_greeting);
		holder.view = (View) view.findViewById(R.id.bubble);
		view.setTag(holder);
		return view;
	}
	...
	public void bindView(View v, int position, RongRedPacketMessage content, UIMessage message) {
		...
		if (message.getMessageDirection() == UIMessage.MessageDirection.SEND) {
			// 消息方向，自己发送的
			holder.view.setBackgroundResource(R.drawable.ease_money_chatto_bg);
		} else {
			// 消息方向，别人发送的
			holder.view.setBackgroundResource(R.drawable.ease_money_chatfrom_bg);
		}
